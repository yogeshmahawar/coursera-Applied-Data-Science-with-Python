...
```python
def answer_one():
    return df[df['Total'] == max(df['Total'])].index[0]
answer_one()
```
...
```python
def answer_five():
#     nw = census_df.set_index(['STNAME', 'CTYNAME'])
    nw = census_df.groupby(['STNAME']).agg({'COUNTY': 'count'})
    return nw[nw['COUNTY'] == max(nw['COUNTY'])].index[0]
answer_five()
```
...
```python
def answer_eight():
    r1_ = census_df[census_df['REGION'] ==1]
    r2_ = census_df[census_df['REGION'] ==2]
    r1_r2 = r1_.append(r2_)
    result_1 = r1_r2[r1_r2['POPESTIMATE2015']> r1_r2['POPESTIMATE2014']] 
    result_1 = result_1[result_1['CTYNAME'].str.contains('Washington')]
    return result_1[['STNAME','CTYNAME'] ]
answer_eight()
```
